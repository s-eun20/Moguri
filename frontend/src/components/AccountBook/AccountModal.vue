    ...
    <div class="modal">
      ...
      <div class="form-group">
        ...
        <div class="input-wrapper">
          <input type="text" id="description" v-model="description" @input="filterGoals" />
          <ul v-if="type === '저축' && filteredGoals.length > 0 && description.trim() !== ''" class="goal-list">
            <li v-for="goal in filteredGoals" :key="goal.goalName" @click="selectGoal(goal)">
              {{ goal.goalName }}
            </li>
          </ul>
        </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const categories = ref(['식비', '교통비', '건강', '쇼핑', '통신비','주거비']);
    ...
    const filterGoals = () => {
      ...
      filteredGoals.value = goalStore.goals.filter(goal => 
  ...
  const newTransaction = {
    ...
    type: type.value,
    category: type.value === '지출' ? category.value : 
              (type.value === '수입' ? incomeType.value : null), // 저축의 경우 category를 null로 설정
    amount: amount.value,
